...
var ExtraNonceCounter = function(configInstanceId){
    ...
    let counter = crypto.randomBytes(4).readUInt32LE(0) + parseInt(configInstanceId || 0);
...
var JobManager = module.exports = function JobManager(options){
    ...
    global.adjustedDiff1 = diff1 * shareMultiplier;
 //public members
    ...
    const nrg_blockash = algos['blockhash'].hash(options.coin);
    ...
    var blockHasher = (function () {
        switch (options.coin.algorithm) {
            ...
            case 'nrghash':
                return function(d) {
                    return util.reverseBuffer(nrg_blockash.apply(this, arguments));
                }
            case 'scrypt-n':
            case 'sha1':
                return function (d) {
    ...
    this.processShare = function(jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName, hashMix, coin, merkleTreeHash){
        ...
        const coinbaseBufferFound = Buffer.from(coin, 'hex');
        const coinbaseBuffer = job.serializeCoinbase(extraNonce1, extraNonce2);
        
        if (!coinbaseBuffer.equals(coinbaseBufferFound)) {
            return shareError([20, `coinbase structure mismatch ${coin} vs ${coinbaseBuffer.toString('hex')} needed`]);
        }
        ...
        if (merkleTreeHash != merkleRoot) {
            return shareError([20, `merkleTreeHash mismatch ${merkleTreeHash} vs ${merkleRoot} needed`]);
        }

        var headerBuffer = job.serializeHeader(merkleRoot, hashMix, nTime, nonce);
        var headerHash = hashDigest(headerBuffer, job.rpcData.version, job.rpcData.previousblockhash, merkleRoot, parseInt(nTime, 16), parseInt(job.rpcData.bits, 16), job.rpcData.height, hashMix, nonce);
        ...
        var shareDiff = adjustedDiff1 / headerBigNum.toNumber();

        var blockDiffAdjusted = job.difficulty * shareMultiplier;
        blockHexInvalid = job.serializeBlock(headerBuffer, coinbaseBuffer).toString('hex');
        blockHashInvalid = blockHasher(headerBuffer, job.rpcData.version, job.rpcData.previousblockhash, merkleRoot, parseInt(nTime, 16), parseInt(job.rpcData.bits, 16), job.rpcData.height, hashMix, nonce).toString('hex');

        //Check if share is a block candidate (matched network difficulty)
        if(job.target.ge(headerBigNum)){
            blockHex = job.serializeBlock(headerBuffer, coinbaseBuffer).toString('hex');
            blockHash = blockHasher(headerBuffer, job.rpcData.version, job.rpcData.previousblockhash, merkleRoot, parseInt(nTime, 16), parseInt(job.rpcData.bits, 16), job.rpcData.height, hashMix, nonce).toString('hex');
        }
        else {
            ...
            if (shareDiff / difficulty < 0.99){
                ...
                if (previousDifficulty && shareDiff >= previousDifficulty){
                    ...
                    return shareError([
                        23,
                        `low difficulty share of ${shareDiff}/${headerHash.toString('hex')} vs ${difficulty} needed`]);
                }
