...
var generateOutputTransactions = function(poolRecipient, recipients, rpcData){
    ...
    if (rpcData.masternode_payments_started) {
        if (rpcData.masternode) {
            if (rpcData.masternode.script) {
                var payeeReward = 0;

                payeeReward = rpcData.masternode.amount;
                rewardToPool -= payeeReward;
                var payeeScript = Buffer.from(rpcData.masternode.script, 'hex');
    ...
    const is_superblock = rpcData.superblocks_enabled;
    if (is_superblock) {
        if (rpcData.superblock.length > 0) {
            for(var i in rpcData.superblock){
                ...
                var payeeScript = Buffer.from(rpcData.superblock[i].script, 'hex');
                txOutputBuffers.push(Buffer.concat([
                            util.packInt64LE(payeeReward),
                            util.varIntBuffer(payeeScript.length),
                            payeeScript
