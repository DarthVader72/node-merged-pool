...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    function OutputPoolInfo(){
        ...
        const multiplier = algos[options.coin.algorithm].multiplier;
        var infoLines = [startMessage,
                ...
                'Current Block Diff:\t' + _this.jobManager.currentJob.difficulty / multiplier,
                'Network Difficulty:\t' + options.initStats.difficulty / multiplier,
    ...
    function DetectCoinData(finishedCallback){
        ...
        _this.daemon.batchCmd(batchRpcCalls, function(error, results){
            if (error || !results){
                ...
                setTimeout(() => DetectCoinData(finishedCallback), 1e3);
                return;
            }
        ...
        _this.stratumServer.on('started', function(){
            ...
            client.on('difficultyChanged', function(diff){
                ...
                if (typeof(options.ports[client.socket.localPort]) !== 'undefined' && options.ports[client.socket.localPort].diff) {
                    difficulty = options.ports[client.socket.localPort].diff;
                }
                
                const {currentJob} = _this.jobManager;

                // Avoid setting more than network difficulty
                if (currentJob && (difficulty > currentJob.difficulty) ) {
                    difficulty = currentJob.difficulty;
                }

                this.enqueueNextDifficulty(difficulty);
                
                if (currentJob) {
                    this.sendMiningJob(currentJob.getJobParams());
                }
                ...
                var result =_this.jobManager.processShare(
                    ...
                    params.name,
                    params.hashMix,
                    params.coinbaseBuffer,
                    params.merkleTreeHash
                );
    ...
    **/
    ...
    this.setVarDiff = function(port, varDiffConfig) {
        ...
        _this.varDiff[port].on('newDifficulty', function(client, newDiff) {
            ...
            // Avoid setting more than network difficulty
            if (currentJob && (newDiff > currentJob.difficulty) ) {
                newDiff = currentJob.difficulty;
            }
